...
contract ERC20 {
    ...
    function getBalanceOf(address walletAddr) public view returns(uint){
    ...
    function transfer(address recipient, uint amount) public { //transfering from own account
    ...
    function approve(address spender, uint amount) public { //allowing other account to spend token on your behalf
        allowance[msg.sender][spender] = amount;
        emit Approval(msg.sender, spender, amount);

    }

    function transferFrom(address sender, address recipient, uint amount) public{ //transfering from other account after approval 
